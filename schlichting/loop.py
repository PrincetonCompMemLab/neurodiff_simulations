...
def loop(output_dir, params, param_names,
    ...
    if depth == len(params):
        ...
        with open(jobfile, "w") as f:
            lines[-3] = f"{analyze_only}./schlichting --mode=batch --saveDirName={saveDirName} --runs={num_exps} --trncyclog=false --tstcyclog=false \
                        {cmd_string} \n"
            lines[-2] = f"python Post_analyses.py {saveDirName} cmd\n"
            lines[-1] = f"python read_task_parameters_into_csv.py {saveDirName} cmd"
...
if __name__ == "__main__":
    ...
    if b'della' in cluster:
        data_dir = f"/scratch/{user}/schlichting"
    ...
    if args.searchvar == 'blocked_interleave_flag':
        param_names = ["--blocked_interleave_flag"]
        blocked_interleave_flag = ['Blocked', 'Interleave']
        params = [blocked_interleave_flag]
