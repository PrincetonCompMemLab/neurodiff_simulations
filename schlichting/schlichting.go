...
var ParamSets = params.Sets{
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "#HiddenToHidden", Desc: "default connection (will change for non-shape learning task).",
				Params: params.Params{
					"Prjn.WtInit.InitStrategy" : "RandomHiddenToHidden", // SparseHiddenToHidden
					"Prjn.WtInit.Var" : ".1",
	...
	{Name: "TaskColorRecall", Desc: "params for the second, color learning task", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: "#HiddenToHidden", Desc: "default connection (will change for non-shape learning task).",
				Params: params.Params{
					...
					"Prjn.Learn.XCal.DMaxMag_NMPH" : "1",
					"Prjn.Learn.XCal.ThrP_NMPH" : ".4", //.5
					"Prjn.WtScale.Abs": "1.9", //1
				    "Prjn.WtInit.Var" : ".05",
					"Prjn.Learn.Lrate": "1",
				}},
			{Sel: "#SceneToHidden", Desc: "default connection (will change for non-shape learning task).",
		...
		{Sel: "#Hidden", Desc: "color output layer turned on for color learning task",
			Params: params.Params{
				"Layer.OscAmnt": ".0623", // was .063 LOWER = .054
				"Layer.Inhib.Layer.Gi": "1", // 2.26
				"Layer.Inhib.InhibType" : "KWTA",
				"Layer.Inhib.K_for_WTA" : "6",
				"Layer.Inhib.K_max" : "12",
		...
		{Sel: "#Scene", Desc: "color output layer turned on for color learning task",
			Params: params.Params{
				"Layer.OscAmnt": ".156", // was .2   LOWER = .178
...
////////////////////////////////////////////////////////////////////////////////////////////
// 		Configs
func (ss *Sim) get_color_diff_StimFile_by_blocked_condition() string {
	hiddentooutput_prjn := ss.getPrjn("Hidden", "Output")
	BlockedInterleaveCondition := hiddentooutput_prjn.WtInit.BlockedInterleaveCondition
	stimfile := fmt.Sprintf("%s/color_diff_stim_%s.dat", ss.StimulusDir, BlockedInterleaveCondition)
...
func (ss *Sim) Config() {
	...
	ss.CurrentStimFile = ss.get_color_diff_StimFile_by_blocked_condition()
}
...
func (ss *Sim) ConfigParamValues() {
	...
	blocked_interleave_flag := ss.Params.SetByName("Base").SheetByName("Network").SelByName("#HiddenToOutput").Params
	ss.SetParamSheetValuefromParamValues(blocked_interleave_flag, "Prjn.WtInit.BlockedInterleaveCondition", "blocked_interleave_flag")
...
func (ss *Sim) NewRndSeed() {
	ss.RndSeed = time.Now().UnixNano()
	// ss.RndSeed = 1643838579619456000
}
...
func (ss *Sim) AlphaCyc(train bool) {
	...
	for qtr := 0; qtr < 4; qtr++ {
		for cyc := 0; cyc < ss.Time.CycPerQtr; cyc++ {
			...
			// if !train {
			// 	hiddenLayer := ss.Net.LayerByName("Hidden").(*leabra.Layer)
			// 	fmt.Println("Gi",ss.TrainEnv.Epoch.Cur,ss.CurrentTest, ss.TestEnv.TrialName.Cur, ss.Time.Cycle, hiddenLayer.Neurons[0].Gi)
			// }
			// if train {
			// 	hiddenLayer := ss.Net.LayerByName("Hidden").(*leabra.Layer)
			// 	fmt.Println("Gi", ss.TrainEnv.Epoch.Cur, ss.CurrentTest, ss.TrainEnv.TrialName.Cur, ss.Time.Cycle, hiddenLayer.Neurons[0].Gi)
			// }
...
func (ss *Sim) TrainTrial() {
	...
	// fmt.Println("Order", ss.TrainEnv.Order)
	// fmt.Println("ss.TrainEnv.Epoch.Cur", ss.TrainEnv.Epoch.Cur)
	hiddentooutput_prjn := ss.getPrjn("Hidden", "Output")
	BlockedInterleaveCondition := hiddentooutput_prjn.WtInit.BlockedInterleaveCondition
	// fmt.Println("ss.TrainEnv.TrialName.Cur", ss.TrainEnv.TrialName.Cur)
	if (ss.TrainEnv.Epoch.Cur == 0) && (BlockedInterleaveCondition == "Interleave") {
		ss.TrainEnv.Order = []int{1, 0}
	...
	// fmt.Println("Order after", ss.TrainEnv.Order)
	...
	// Skip med1 trials if in blocked condition
	if (BlockedInterleaveCondition == "Blocked") && (ss.TrainEnv.TrialName.Cur == "med1") {
		return
	}
...
func (ss *Sim) TestColorAll() {
	...
	ss.SetUpTestTrial(ss.get_color_diff_StimFile_by_blocked_condition(), "TaskColorRecall", "TestColorAll")
...
func (ss *Sim) TestSceneAll() {
	...
	ss.SetUpTestTrial(ss.get_color_diff_StimFile_by_blocked_condition(), "TaskColorRecall", "TestSceneAll")
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	win := gi.NewMainWindow("schlichting", "Schlichting Model", width, height)
	ss.Win = win
...
func (ss *Sim) CmdArgs() (err error) {
	...
	CreateFlagVariable("blocked_interleave_flag", "string", nil, "block or interleave condition")
...
func GenerateWeights( strategy string, pj *leabra.Prjn, array_len int, ri int) (arr []float32) {
	...
	if (pj.WtInit.NumOverlapUnits != 2) || (pj.WtInit.NumTotalUnits != 6) {
		panic("In Schlichting experiment model, NumOverlapUnits must be 2 and NumTotalUnits must be 6")
	}
	...
	if strategy == "Rand" {
		...
		BlockedInterleaveCondition := pj.WtInit.BlockedInterleaveCondition
		// var StrongConnection, WeakConnection float32
		var StrongConnection float32
		var WeakConnection float32

		if BlockedInterleaveCondition == "Blocked" {
			StrongConnection = float32(pj.WtInit.StrongConnection)
			WeakConnection = float32(pj.WtInit.WeakConnection)
		} else if BlockedInterleaveCondition == "Interleave" {
			StrongConnection = 0.8
			WeakConnection = float32(pj.WtInit.WeakConnection)

		} else {
			panic(fmt.Sprintf("BlockedInterleaveCondition: %s not supported", BlockedInterleaveCondition))
		}

		// StrongConnection = float32(pj.WtInit.StrongConnection)
		// WeakConnection = float32(.2)


		for ci := 0; ci < array_len; ci++ {
			arr = append(arr, float32(pj.WtInit.Gen(-1)))
			// arr = append(arr, WeakConnection + float32(erand.UniformMeanRange(0, 0.01, -1)))
		}

		if ri == 25 {
			for ci := 0; ci < 50; ci++ {
				if (minunitid_item1 <= ci) && (ci < maxunitid_item1) {
					arr[ci] = StrongConnection
					}  else if (minunitid_item2 <= ci) && (ci < maxunitid_item2) {
						arr[ci] = WeakConnection
				} else {
					// arr[ci] = WeakConnection + float32(erand.UniformMeanRange(0, 0.01, -1))
					arr[ci] = arr[ci]
				}
			}
		}
...
func (ss *Sim) ReloadStimFiles() (err error) {
	...
	write_name := fmt.Sprintf("%s/color_diff_stim_%s.dat", ss.StimulusDir, hiddentooutput_prjn.WtInit.BlockedInterleaveCondition)
